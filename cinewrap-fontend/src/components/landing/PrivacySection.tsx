        ...
        >
          {PRIVACY_ACCORDION_DATA.map((item) => (
            ...
            >
              ...
              >
                <div
                  className={`flex-shrink-0 p-2 rounded-lg transition-colors ${openPrivacy === item.id ? "bg-cine-primary/20 text-cine-primary" : "bg-white/5 text-cine-text-muted group-hover:bg-cine-primary/10 group-hover:text-cine-primary"}`}
                >
                  {item.icon}
                </div>
                <div className="flex-1 min-w-0">
                  <p
                    className={`font-bold text-sm md:text-base transition-colors ${openPrivacy === item.id ? "text-cine-primary" : "text-cine-text group-hover:text-cine-primary/80"}`}
                  >
                    {item.title}
                  </p>
                ...
                <svg
                  className={`w-5 h-5 transition-transform duration-300 ${openPrivacy === item.id ? "rotate-180 text-cine-primary" : "text-cine-text-muted"}`}
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
              ...
              <AnimatePresence>
                {openPrivacy === item.id && (
                  ...
                  >
                    {/* XỬ LÝ MA THUẬT: In trực tiếp nội dung JSX cực kỳ phức tạp đã cất trong landingData ra đây! */}
